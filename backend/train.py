...
create_tables()
from database import save_prediction, save_metrics
import time
...
class Config:
    ...
    PRETRAINED_MODEL_PATH = "model_weights/modified_model.pth" #FG-BERT
    CHECKPOINT_DIR = "ckpts"

    RESULTS_CSV = "ABFormer_results.csv"
    LOG_DIR = "ABFormer_logs"
    ...
    BATCH_SIZE = 1
    MAX_EPOCHS = 5
    MIN_CHECKPOINT_EPOCH = 3 # Prevent early stopping/checkpointing in the first  epochs because the model shows high AUC before meaningful learning.
...
def evaluate_model(model, data_loader, device, criterion):
    ...
    with torch.no_grad():
        for x1, x1maccs, x2, x2maccs, t1, t2, t3, aac1, aac2, aac3, t4, labels in data_loader:
            ...
            outputs, attn_weights = model(x1, x1maccs, x2, x2maccs, t1, t2, t3, aac1, aac2, aac3, t4, 
...
def train_one_seed(seed, config, device, UNIQUE_SPLIT):
    ...
    for epoch in epoch_pbar:
        ...
        for x1, x1maccs, x2, x2maccs, t1, t2, t3, aac1, aac2, aac3, t4, labels in batch_pbar:
            ...
            outputs, _ = model(x1, x1maccs, x2, x2maccs, t1, t2, t3, aac1, aac2, aac3, t4,
                   mask1=None, adjoin_matrix1=None, mask2=None, adjoin_matrix2=None)
...
#     )
        ...
        if epoch >= config.MIN_CHECKPOINT_EPOCH:
            ...
            curr_prauc = float(val_metrics['pr_auc'])
            curr_auc = float(val_metrics['roc_auc'])
            curr_acc = float(val_metrics['accuracy'])
            ...
            print("DEBUG:", curr_auc, best_auc, is_better)
            if is_better:
    ...
    model.eval()
    with torch.no_grad():
        for x1, x1maccs, x2, x2maccs, t1, t2, t3, aac1, aac2, aac3, t4, labels in test_loader:
            x1, x2 = x1.to(device), x2.to(device)
            t1, t2, t3, t4 = t1.to(device), t2.to(device), t3.to(device), t4.to(device)
            x1maccs, x2maccs = x1maccs.to(device), x2maccs.to(device)
            aac1, aac2, aac3 = aac1.to(device), aac2.to(device), aac3.to(device)
