...
class AB_Data:
    ...
    def cover_dict(self, path):
        ...
        return tensor_dict       # new_data = {i : value for i,(key,value) in enumerate(tensor_dict.items())}
            # return new_data
    ...
    def get_dataloaders_from_csv(self, train_csv='data/train.csv', val_csv='data/val.csv', test_csv='data/test.csv', batch_size=40):
            ...
            def create_dataset_from_ids(id_list, split_name="Split"):
                ...
                t1 = torch.tensor(np.array([x.detach().cpu() for x in t1]))           
                t2 = torch.tensor(np.array([x.detach().cpu() for x in t2]))                
                t3 = torch.tensor(np.array([x.detach().cpu() for x in t3]))              
                t4 = torch.tensor(np.array([x.detach().cpu() for x in t4]))                
